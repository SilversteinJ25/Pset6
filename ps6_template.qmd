...
```{python}
jam_scatter_plot = alt.Chart(top_jam_alerts).mark_circle(size=100).encode(
    x=alt.X('Longitude:Q', scale=alt.Scale(domain=[top_jam_alerts['Longitude'].min(), top_jam_alerts['Longitude'].max()]), title='Longitude'),  # Longitude should be on the x-axis
    y=alt.Y('Latitude:Q', scale=alt.Scale(domain=[top_jam_alerts['Latitude'].min(), top_jam_alerts['Latitude'].max()]), title='Latitude'),  # Latitude should be on the y-axis
    size='Alert_Count:Q', 
...
It would be a good idea because it would make the dashboard more user friendly and easier to understand the data. While some nuance in patterns may be lost ass well as precision due to it being by the hour and not minute, it would still show a pattern that is close to the overall data.
...
I would include what year this is from and more tools that could allow a user to see different alerts over time. Alerts across the city alone are good but tell us little without further comparisons. As well as tools that could allow a user to narrow in on specific parts of the city to make cross city comparisons. Furthermore, it would be a good idea to allow people to change the colors of and data points to allow for better visualization.
