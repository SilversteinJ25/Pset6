...
c. Finally, do you consider that we should keep the NA subt
...
```{python}
crosswalk = full_waze_data.groupby('type')['subtype'].unique().reset_index()
crosswalk.columns = ['Type', 'Subtypes']
print("Crosswalk table (Type vs. Subtype):")
print(crosswalk)
```

```{python}
def categorize_subtypes_by_keywords(subtypes, keywords):
    """Categorize and sort subtypes based on keyword matching."""
    categorized = {keyword: [] for keyword in keywords}
    uncategorized = []

    for subtype in subtypes:
        matched = False
        for keyword in keywords:
            if isinstance(subtype, str) and keyword.lower() in subtype.lower():
                categorized[keyword].append(subtype)
                matched = True
                break
        if not matched:
            uncategorized.append(subtype)
    for keyword in categorized:
        categorized[keyword].sort()
    return categorized, uncategorized

keywords = ['hazard', 'accident']
cleaned_hierarchy = {}

for type_name, subtypes in full_waze_data.groupby('type')['subtype']:
    categorized_subtypes, uncategorized_subtypes = categorize_subtypes_by_keywords(subtypes, keywords)
    cleaned_hierarchy[type_name] = {
        "Hazard": categorized_subtypes.get("hazard", []),
        "Accident": categorized_subtypes.get("accident", []),
        "Uncategorized": uncategorized_subtypes
    }

print("Cleaned Hierarchical Structure:")
for type_name, hierarchy in cleaned_hierarchy.items():
    print(f"- {type_name}")
    for category, subtypes in hierarchy.items():
        print(f"  - {category}: {', '.join(subtypes) if subtypes else 'None'}")
...
b. Let each row of this DataFrame be a unique combination of type and subtype. Then, based on the hierarchy you proposed in Q3, fill in updated_type, updated_subtype, and updated_subsubtype accordingly. Remember to name the
NA subtypes as “Unclassified”. Hint: your crosswalk should have 32 observations.
