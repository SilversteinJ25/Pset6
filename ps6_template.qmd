...
Four typess have a subtype with NAs. The subtypes can be seen in the below code's output. Of the four types, Hazard has 19 subtypes, while the others all have less than 5, so it is worth giving it subtypes.
...
full_waze_data['cleaned_subtype'] = full_waze_data.apply(clean_subtypes, axis=1)

clean_crosswalk = full_waze_data.groupby('type')['cleaned_subtype'].unique().reset_index()

for _, row in clean_crosswalk.iterrows():
    print(f"{row['type']}:")
    for subtype in row['cleaned_subtype']:
        print(f"  - {subtype}")
```
...
In real life, rare events can happen that are not easily classified into premade categories. Or we have labeling issues from person to perrson on how to categorize thingss, so it is left blank. It is a good idea to have an unclassified subtype in such cases.

```{python}
full_waze_data['cleaned_subtype'] = full_waze_data['cleaned_subtype'].fillna('UNCLASSIFIED')

print(full_waze_data[['type', 'subtype', 'cleaned_subtype']].head())
```
